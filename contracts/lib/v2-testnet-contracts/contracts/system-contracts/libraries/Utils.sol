...
library Utils {
    ...
    /// - Bytecode words length is not odd
    function hashEVMBytecode(uint256 _evmBytecodeLen, bytes calldata _paddedBytecode)
        internal
        view
        returns (bytes32 hashedEVMBytecode)
    {
        // Note that the length of the bytecode must be provided in 32-byte words.
    ...
    function getNewAddressCreate2EVM(address _sender, bytes32 _salt, bytes32 _bytecodeHash)
        internal
        pure
